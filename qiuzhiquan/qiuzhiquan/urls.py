...
urlpatterns = [
    ...
    path('register/', RegView.as_view(), name="register"),  # 用户注册映射
    path('login/', LoginView.as_view(), name="login"),  # 用户登录映射
    path('logout/', LogoutView.as_view(), name="logout"),  # 用户注销映射
    path('contact/', ContactView.as_view(), name="contact"),  # 联系页面的映射
    path('faq/', FaqView.as_view(), name="faq"),  # 问答页面的映射
    path('', IndexView.as_view(), name="index"),  # 首页映射
    re_path(r'^media/(?P<path>.*)$', serve, {"document_root": MEDIA_ROOT}),  # 管理媒体文件路径和处理模块
    re_path(r'^addcomment/(?P<interview_id>\d+)/$', AddCommentView.as_view(), name="addcomment"),  # 评论映射
    re_path(r'^interdetail/(?P<interview_id>\d+)/$', InterDetailView.as_view(), name="interdetail"),  # 面经详情映射
]
