...
from django.http import HttpResponseRedirect, HttpResponse, request  # 导入重定向模块（HttpResponse不属于重定向）
from django.shortcuts import render, redirect, render_to_response
from django.contrib.auth.hashers import make_password  # 导入密码加密模块
from django.urls import reverse  # 导入reverse解析模块（用来重定向）
from interview.forms import RegForm, LoginForm, CommentForm  # 导入用户表单（注册/登录/评论）
from django.views import View
from interview.models import Interview, Author, User, Comment, Faq
from pure_pagination import Paginator, EmptyPage, PageNotAnInteger  # 导入分页块的类
...
class InterListView(View):
    ...
    def get(self, request):
        ...
        # 基于年级的分类(基于年级找作者，基于作者找面经)
        year = request.GET.get('year', "")  # 获取关键词year，若没有则设置为空
        if year:
            authors = Author.objects.filter(year=year)  # 取出这个年级对应的作者们
            all_interviews = Interview.objects.none()  # 设置空面经
            for author in authors:
                interviews = author.interview_set.all()  # 获取每个作者对应的面经（author和interview存在一对多的外键关系，可以调用interview_set方法）
                all_interviews = all_interviews | interviews  # 每个作者的面经合并起来（queryset）
        ...
        return render(request, "interview_list.html", {
            "all_interviews": interviews,  # 传递模板变量给前面的模板文件
        })
...
class InterDetailView(View):
    ...
    def get(self, request, interview_id):  # 传入interview_id参数（此参数通过url映射得到）
        interview = Interview.objects.get(id=int(interview_id))  # 根据面经id得到面经
    ...
    def post(self, request, interview_id):
        ...
        if request.user.is_authenticated:  # 判断用户是否登录
            ...
            if comment_form.is_valid():  # 判断表单输入是否有效
                ...
                comments.save()  # 保存面经
                return redirect(request.META['HTTP_REFERER'])  # 若表单内容已经保存，则刷新页面
            else:
                return HttpResponse("请输入正确的评论内容！")  # 若表单信息无效，给用户错误报告
        else:  # 如果登录失效或者用户未登录，则重定向/返回登录页面
            return render(request, "login.html", {
                # 若用户没有登录，则跳转到登录页面
                "error": "请登录后再评论"
            })
...
class FaqView(View):
    ...
    def get(self, request):
        all_faqs = Faq.objects.all()  # 获取所有的问答
